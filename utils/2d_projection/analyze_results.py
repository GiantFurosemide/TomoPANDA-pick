...
def process_star_txt_tbl(
    ...
    index_file = (output_dir / "index.txt").resolve()  # 转换为绝对路径
    print(f"Step 1: Saving index file to: {index_file}")
    indices_1based = extract_slice_indices_from_star(star_file, index_file)
    print(f"Step 1: Extracted {len(indices_1based)} unique slice indices from star file")
    # 验证文件是否创建（使用resolve后的路径）
    if index_file.exists():
        print(f"Step 1: ✓ Index file created successfully: {index_file}")
        print(f"Step 1: File size: {index_file.stat().st_size} bytes")
    else:
        print(f"Step 1: ✗ WARNING: Index file not found at: {index_file}")
    ...
    output_particle_txt = (output_dir / f"{particle_txt_basename}.processed{particle_txt_ext}").resolve()  # 转换为绝对路径
    print(f"Step 2: Saving particle txt file to: {output_particle_txt}")
    ...
    if output_particle_txt.exists():
        print(f"Step 2: ✓ Particle txt file created successfully: {output_particle_txt}")
        print(f"Step 2: File size: {output_particle_txt.stat().st_size} bytes")
    else:
        print(f"Step 2: ✗ WARNING: Particle txt file not found at: {output_particle_txt}")
    ...
    output_tbl = (output_dir / f"{tbl_basename}.processed{tbl_ext}").resolve()  # 转换为绝对路径
    print(f"Step 4: Saving tbl file to: {output_tbl}")
    ...
    if output_tbl.exists():
        print(f"Step 4: ✓ TBL file created successfully: {output_tbl}")
        print(f"Step 4: File size: {output_tbl.stat().st_size} bytes")
    else:
        print(f"Step 4: ✗ WARNING: TBL file not found at: {output_tbl}")
